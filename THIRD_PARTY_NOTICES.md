...
## macOS editing-command table (Playwright)

`browser/extension/background.js` contains `MAC_EDITING_COMMANDS`, the editing command each key
chord runs on macOS. It is ported entry for entry from Playwright's
[`macEditingCommands.ts`](https://github.com/microsoft/playwright/blob/main/packages/playwright-core/src/server/macEditingCommands.ts),
retrieved 2026-09-19. The only change is the form: a TypeScript object literal became a
prototype-less JavaScript object; every chord and every value is unchanged.

That file is Copyright 2017 Google Inc., with modifications copyright Microsoft Corporation, and is
licensed under the Apache License, Version 2.0. Playwright's `NOTICE` file reads:
