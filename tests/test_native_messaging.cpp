...
void NativeMessagingTests::encode_prefixesLittleEndianLength() {
  ...
  QCOMPARE(frame.size(),
           16); // 4-byte length prefix + 12-byte compact JSON {"type":"x"}
...
void NativeMessagingTests::parse_shortBufferNeedsMore() {
  ...
  QCOMPARE(parseFrame(frame.left(frame.size() - 1)).status,
           NativeFrame::Status::NeedMore);
}
...
void NativeMessagingTests::parse_oversizedLengthIsError() {
  QByteArray frame;
  const quint32 tooBig = qToLittleEndian<quint32>(
      static_cast<quint32>(kMaxNativeMessageBytes) + 1);
  frame.append(reinterpret_cast<const char *>(&tooBig), 4);
  const NativeFrame parsed = parseFrame(frame);
  QCOMPARE(parsed.status, NativeFrame::Status::Error);
  QCOMPARE(parsed.error,
           QStringLiteral(
               "Native message length 67108865 exceeds the 67108864-byte cap"));
}

void NativeMessagingTests::parse_exactlyCapSizedLengthIsAccepted() {
  // The oversize guard is strictly greater-than, so a frame whose declared body
  // length is EXACTLY the cap is legal and must be accepted. This pins that
  // boundary: a >= mutation would wrongly reject a maximum-sized message. Body
  // is a valid JSON object of exactly kMaxNativeMessageBytes bytes
  // ({"p":"aa...a"} -> 8 bytes of fixed overhead).
...
void NativeMessagingTests::parse_nonObjectBodyIsError() {
  const QByteArray body =
      QByteArrayLiteral("[1,2,3]"); // valid JSON, but an array
  QByteArray frame;
  ...
  QVERIFY(parsed.error.startsWith(
      QStringLiteral("Native message is not a JSON object: ")));
}
