...
QJsonObject toolCall(const QString& name, const QJsonValue& arguments = QJsonObject{}) {
    return request(QStringLiteral("tools/call"),
                   3,
...
void BrowserMcpServerTests::fullCatalogContainsEveryBrowserAndInstallerTool() {
    ...
    const auto response = handleRequest(request(QStringLiteral("tools/list"), 2), &browser);
    QVERIFY(response.has_value());
...
void BrowserMcpServerTests::readOnlyCatalogFiltersMutatingTools() {
    ...
    const auto refused = handleRequest(toolCall(QStringLiteral("browser_click")), &browser, policy);
    QVERIFY(refused.has_value());
    QCOMPARE(refused->value(QStringLiteral("error")).toObject().value(QStringLiteral("code")).toInt(),
             -32602);
}

void BrowserMcpServerTests::notificationGetsNoResponse() {
    const auto response = handleRequest(
        request(QStringLiteral("notifications/initialized"), QJsonValue::Undefined));
    QVERIFY(!response.has_value());
}

void BrowserMcpServerTests::malformedEnvelopeIsRejected() {
    QJsonObject malformed{{QStringLiteral("jsonrpc"), QStringLiteral("1.0")},
                          {QStringLiteral("id"), 8},
...
void BrowserMcpServerTests::nonObjectArgumentsAreRejected() {
    const auto response =
        handleRequest(toolCall(QStringLiteral("browser_extension_status"), QStringLiteral("bad")));
    QVERIFY(response.has_value());
    QCOMPARE(response->value(QStringLiteral("error")).toObject().value(QStringLiteral("code")).toInt(),
             -32602);
}
...
void BrowserMcpServerTests::installerStatusReturnsMcpContent() {
    const auto response = handleRequest(toolCall(QStringLiteral("browser_extension_status")));
    QVERIFY(response.has_value());
    const QJsonObject result = response->value(QStringLiteral("result")).toObject();
...
void BrowserMcpServerTests::imageResultUsesMcpImageBlock() {
    const QJsonObject result = toolCallResult({.text = QStringLiteral("captured"),
                                               .is_error = false,
