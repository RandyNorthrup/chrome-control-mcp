...
namespace {
// Every native-messaging frame is prefixed with a little-endian uint32 length
// header; this is that header's width in bytes.
...
NativeFrame parseFrame(const QByteArray &buffer) {
  ...
  if (length > static_cast<quint32>(kMaxNativeMessageBytes)) {
    frame.status = NativeFrame::Status::Error;
    frame.error =
        QStringLiteral("Native message length %1 exceeds the %2-byte cap")
            .arg(length)
