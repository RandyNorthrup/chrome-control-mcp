...
bool resolveReadOnlyProfile(const QString& raw) {
    ...
    qWarning(
        "CHROME_CONTROL_MCP_SECURITY_PROFILE='%s' is not a recognized profile; failing closed to "
        "read_only.",
        qUtf8Printable(raw));
    return true;
}
...
bool resolveRedactSensitiveOutput(const QString& raw) {
    ...
    qWarning(
        "CHROME_CONTROL_MCP_REDACT_SENSITIVE_OUTPUT='%s' is not a recognized value; failing closed to "
        "redacting.",
        qUtf8Printable(raw));
    return true;
}
...
QJsonObject initializeResult() {
    return QJsonObject{
        {QStringLiteral("protocolVersion"), QString::fromLatin1(mcp::kProtocolVersion)},
...
Win32McpServerPolicy Win32McpServerPolicy::fromEnvironment() {
    ...
    policy.read_only_profile =
        resolveReadOnlyProfile(env.value(QStringLiteral("CHROME_CONTROL_MCP_SECURITY_PROFILE")));
