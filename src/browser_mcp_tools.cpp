...
QString installerError(const ExtensionInstallResult& result) {
    return result.detail.isEmpty() ? result.summary
...
QJsonArray toolCatalog() {
    return QJsonArray{
        toolEntry(
            ...
            QStringLiteral(
                "Prepare the unpacked Chrome Control MCP browser extension by registering its "
                "current-user native messaging host. Returns the exact folder to load once from "
                "chrome://extensions; no package or private key is required.")),
        toolEntry(
            QStringLiteral("browser_extension_uninstall"),
            QStringLiteral(
                "Remove the Chrome Control MCP native messaging host registration. If the unpacked "
                "extension is loaded, remove it manually from chrome://extensions.")),
        toolEntry(
            QStringLiteral("browser_extension_status"),
            QStringLiteral(
                "Report unpacked-extension availability, native-host preparation state, its load "
