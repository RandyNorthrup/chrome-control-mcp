...
HANDLE relayConnect(const QString& rendezvous_path,
    ...
    if (!pidIsOwnImage(static_cast<DWORD>(record.app_pid))) {
        setError(error,
                 QStringLiteral("Bridge server pid %1 is not the Chrome Control MCP binary (or is gone).")
                     .arg(record.app_pid));
        return INVALID_HANDLE_VALUE;
    }
    ...
    if (GetNamedPipeServerProcessId(pipe, &server_pid) == FALSE ||
        ...
        setError(error, QStringLiteral("The bridge pipe is not served by the Chrome Control MCP binary."));
        CloseHandle(pipe);
        return INVALID_HANDLE_VALUE;
    }
