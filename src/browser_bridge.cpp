...
// Where a browser_print PDF lands: the user's Downloads under a ChromeControlMCP subfolder -- the
...
BrowserBridgeSession::Outgoing BrowserBridgeSession::beginCommand(const QString& tool,
    ...
    if (!connected_) {
        return refuse(QStringLiteral(
            "Browser not connected: the Chrome Control MCP browser-control extension is not attached."));
    }
