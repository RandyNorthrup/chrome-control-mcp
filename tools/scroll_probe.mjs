...
for (let i = 0; i < 120 && !wsUrl; i++) {
  ...
  try {
    ...
    // Not written yet; the loop is the wait.
  }
}
if (!wsUrl) {
...
const found = await cdp.send(
  "DOM.querySelector",
  { nodeId: doc.root.nodeId, selector: "#scroll-region" },
  sessionId,
);
...
console.log(
  "chrome        ",
  (await cdp.send("Browser.getVersion", {})).product,
);
console.log(
  "environment   ",
  JSON.stringify(
...
for (let i = 0; i < 5; i++) {
  ...
  console.log(
    `run ${i}: immediate y=${immediate.y.toFixed(1)} (hits ${hitNow})` +
...
const variants = {
  "wheel at the shipping point": async () => {
    ...
    await cdp.send(
      ...
      { type: "mouseWheel", x: pt.x, y: pt.y, deltaX: 0, deltaY: 180 },
      sessionId,
    );
  },
  ...
  "four wheels of 45": async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await cdp.send(
        "Input.dispatchMouseEvent",
        { type: "mouseWheel", x: pt.x, y: pt.y, deltaX: 0, deltaY: 45 },
        sessionId,
      );
      await sleep(30);
    }
...
for (const [name, run] of Object.entries(variants)) {
  ...
  for (let i = 0; i < 5; i++) {
    try {
      ...
      out.push("ERR:" + e.message.slice(0, 50));
      continue;
    }
    await sleep(800);
