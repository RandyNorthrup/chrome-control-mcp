...
/// server's environment (CHROME_CONTROL_MCP_SECURITY_PROFILE, CHROME_CONTROL_MCP_REDACT_SENSITIVE_OUTPUT). The
...
struct Win32McpServerPolicy {
    // CHROME_CONTROL_MCP_SECURITY_PROFILE == "read_only": tools/list advertises only read-only tools and
    ...
    // CHROME_CONTROL_MCP_REDACT_SENSITIVE_OUTPUT == "true": mask the secret assignments
