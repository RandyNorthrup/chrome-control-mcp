...
function send(reply) {
  if (!port) {
    console.warn("[ChromeControlMCP] no port to reply on; dropping", reply && reply.id);
    return;
  }
  try {
...
function onHostMessage(msg) {
  ...
  if (msg.type === "command") {
    ...
    if (typeof msg.id !== "string" || msg.id.length === 0) {
      console.warn("[ChromeControlMCP] command frame with no usable id; dropping");
      return;
    }
...
async function releaseCdpObjects() {
  ...
  await sendCdp(tabId, "Runtime.releaseObjectGroup", { objectGroup: CDP_OBJECT_GROUP })
    .catch((e) => { console.warn("[ChromeControlMCP] releaseObjectGroup failed:", e); });
}
...
async function handleScreenshot(tabId, args) {
  ...
  const includeControlOverlay = !!(args && args.include_control_overlay === true);
...
function setTabControlBadge(tabId, on) {
  ...
  try {
    if (on) {
      ...
      settle(chrome.action.setTitle({ tabId, title: "Chrome Control MCP assistant is controlling this tab" }));
    } else {
...
function documentHitPoint(x, y, state) {
  if (!state || state.ok !== true || !Number.isFinite(x) || !Number.isFinite(y) ||
      !Number.isFinite(state.scrollX) || !Number.isFinite(state.scrollY)) {
    return null;
  }
  return {
...
async function storageOperation(tabId, storageId, area, action, key, value) {
  ...
  if (!frameId) {
    throw new Error("Could not identify the active document for browser_storage.");
  }
  ...
  const operation = function (area, action, key, value, expectedOrigin, maxValue, maxKeys, maxKey) {
    try {
      if (location.origin !== expectedOrigin) {
        return { ok: false, error: "The document origin changed before the storage operation." };
      }
      var store = area === "session" ? globalThis.sessionStorage : globalThis.localStorage;
      var proto = globalThis.Storage.prototype;
      var get = function (k) { return proto.getItem.call(store, k); };
      var length = function () {
        return Object.getOwnPropertyDescriptor(proto, "length").get.call(store);
      };
      if (action === "get") {
        var got = get(key);
        if (got === null) { return { ok: true, present: false, value: null }; }
        var text = String(got);
        return text.length > maxValue
          ? { ok: true, present: true, value: text.slice(0, maxValue), truncated: true }
      ...
      if (action === "keys") {
        ...
        for (var i = 0; i < count; i++) {
          ...
          if (raw.length > maxKey) { cut = true; raw = raw.slice(0, maxKey); }
          names.push(raw);
        }
        return { ok: true, keys: names, count: total, capped: total > maxKeys,
                 keys_truncated: cut };
      }
      if (action === "set") {
        proto.setItem.call(store, key, value);
        if (get(key) !== value) {
          return { ok: false, error: "The value was not stored (the area rejected or dropped the write)." };
        }
      ...
      if (action === "remove") {
        ...
        if (get(key) !== null) { return { ok: false, error: "The key is still present after the remove." }; }
      ...
      var after = length();
      if (after > 0) {
        return { ok: false, error: "The storage area still holds " + after + " entries after the clear." };
      }
  ...
  const call = await sendCdp(tabId, "Runtime.callFunctionOn", {
    ...
    arguments: [
      { value: area }, { value: action }, { value: key }, { value },
      { value: storageId.securityOrigin }, { value: MAX_STORAGE_VALUE_CHARS },
      { value: MAX_STORAGE_KEYS }, { value: MAX_STORAGE_KEY_CHARS },
    ],
    returnByValue: true,
  });
  const result = call && call.result && call.result.value;
  if (call && call.exceptionDetails) {
    throw new Error("The isolated storage operation threw before returning a result.");
  }
  if (!result || result.ok !== true) {
    throw new Error((result && result.error) || "browser_storage failed in its isolated context.");
  }
  return result;
}
...
async function handleStorage(tabId, args) {
  ...
  const result = await storageOperation(
    tabId, storageId, area, action, args.key || "", action === "set" ? args.value : "");
